...
function Simulation({ snapshot, selectedMetric, selectedLocation, setSelectedLocation, activeLayer, setActiveLayer, speed, setSpeed, toggleSimulation, resetSimulation, setView, aiStatus }: { snapshot: Snapshot | null; selectedMetric: Snapshot['locations'][number] | undefined; selectedLocation: string; setSelectedLocation: (value: string) => void; activeLayer: LayerKey; setActiveLayer: (value: LayerKey) => void; speed: number; setSpeed: (value: number) => void; toggleSimulation: () => void; resetSimulation: () => void; setView: (value: 'setup' | 'simulation' | 'report') => void; aiStatus: AIStatus | null }) {
  return <section className="simulation-page">
    <div className="page-heading"><div><div className="eyebrow muted"><span className="live-dot" />LIVE SIMULATION</div><h1>SoHo is in motion.</h1><p>Watch behavior, not just foot traffic.</p></div><div className="simulation-actions"><WeatherBadge weather={snapshot?.weather} /><TransitBadge transit={snapshot?.transit} /><DataSourceBadges sources={snapshot?.data_sources} /><button className="secondary-button" onClick={resetSimulation}><RotateCcw size={15} /> Reset</button><button className="primary-button" onClick={toggleSimulation}>{snapshot?.running ? <><Pause size={15} fill="currentColor" /> Pause twin</> : <><Play size={15} fill="currentColor" /> Start twin</>}</button></div></div>
...
function WeatherBadge({ weather }: { weather?: WeatherInfo }) { if (!weather) return null; const condEmoji: Record<string, string> = { clear: '☀️', overcast: '☁️', rain: '🌧️', drizzle: '🌦️', snow: '❄️', hot: '🔥', cold: '🥶', fog: '🌫️', thunderstorm: '⛈️', heavy_rain: '🌧️', freezing_rain: '🧊', heavy_snow: '❄️' }; const modPct = Math.round((weather.traffic_modifier - 1) * 100); return <div className="weather-badge"><span className="weather-emoji">{condEmoji[weather.condition] ?? '🌤️'}</span><div><strong>{Math.round(weather.temperature_f)}°F</strong><span className={`weather-mod ${modPct >= 0 ? 'positive' : 'negative'}`}>{modPct >= 0 ? '+' : ''}{modPct}% traffic</span></div></div> }
function TransitBadge({ transit }: { transit?: TransitInfo }) { if (!transit) return null; const statusEmoji: Record<string, string> = { good: '🟢', minor_delays: '🟡', major_delays: '🔴' }; const modPct = Math.round((transit.traffic_modifier - 1) * 100); const affected = transit.lines_affected.length > 0 ? transit.lines_affected.join(', ') : 'none'; return <div className="transit-badge"><span className="transit-emoji">{statusEmoji[transit.status] ?? '🟢'}</span><div><strong>MTA: {transit.status.replace('_', ' ')}</strong><span className={`transit-mod ${modPct >= 0 ? 'positive' : 'negative'}`}>{transit.lines_affected.length > 0 ? `Lines ${affected}: ${modPct}% subway traffic` : 'All lines running'}</span></div></div> }
function DataSourceBadges({ sources }: { sources?: { weather: string; demographics: string; nyc_open_data: string; consumer_feed: string } }) { if (!sources) return null; const entries: [string, string][] = [['weather', sources.weather], ['demographics', sources.demographics], ['nyc_open_data', sources.nyc_open_data], ['consumer_feed', sources.consumer_feed]]; const labels: Record<string, string> = { weather: 'Open-Meteo', demographics: 'US Census', nyc_open_data: 'NYC 311', consumer_feed: 'AI Feed' }; return <div className="data-source-badges">{entries.map(([key, value]) => <span key={key} className={`source-badge ${value === 'fallback' ? 'fallback' : 'live'}`}>{labels[key]}: {value}</span>)}</div> }
